...
module.exports = function(connect) {
  ...
  function MarkLogicStore(options) {
    ...
    this.baseUri = options.baseUri || encodeUriComponent(collectionName.replace(/\s/g,'-'));
   ...
   */
   ...
   function documentsCallback(callback,max) {
    ...
    return function(documents) {
      if (max && documents && documents.length > max) {
  ...
  MarkLogicStore.prototype.set = function(sid, session, callback) {
    ...
    this.db.write({ uri: getDocumentUri(sid, this.baseUri), collections: [ this.collectionName ], content: s}).result(writeCallback(callback));
  ...
  MarkLogicStore.prototype.destroy = function(sid, callback) {
    ...
    this.db.remove(getDocumentUri(sid,this.baseUri)).result().then(function() { callback(); }).catch(callback);
  };
  ...
  MarkLogicStore.prototype.length = function(callback) {
    var q = marklogic.queryBuilder;
    this.db.count(q.collection(this.collectionName)).result(documentsCallback(callback));
  };
  ...
  MarkLogicStore.prototype.clear = function(callback) {
    this.db.removeAll({ collection: this.collectionName }).result().then(writeCallback(callback)).catch(callback);
  };
