...
var deprecate = require('depd')('connect-marklogic');
...
module.exports = function(connect) {
  ...
  function MarkLogicStore(options) {
    ...
    self.db = marklogic.connect(options);
   ...
   */
   ...
   /**
     ...
     */
   function documentsCallback(callback) {
  ...
  MarkLogicStore.prototype.get = function(sid, callback) {
    ...
    var self = this;
    // get a session
    var q = self.db.queryBuilder;
    q.documents.query(
      q.where(
        q.collection(self.collectionName),
        q.value('sid', sid)
      )
    ).result(documentsCallback(callback));
  ...
  MarkLogicStore.prototype.set = function(sid, session, callback) {
    ...
    this.db.documents.write(s).result(writeCallback(callback));
  ...
  MarkLogicStore.prototype.destroy = function(sid, callback) {
    sid = this.hash ? crypto.createHash(this.hash.algorithm).update(this.hash.salt + sid).digest('hex') : sid;
    var q = this.db.queryBuilder;
    this.db.documents.remove(
      q.collection(this.collectionName),  
      q.value('sid', sid)
    ).result().then(function() { callback(); }).catch(callback);
  };
